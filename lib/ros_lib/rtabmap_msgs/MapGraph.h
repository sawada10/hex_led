#ifndef _ROS_rtabmap_msgs_MapGraph_h
#define _ROS_rtabmap_msgs_MapGraph_h
...
namespace rtabmap_msgs
{
  ...
  class MapGraph : public ros::Msg
  {
    public:
      ...
      typedef rtabmap_msgs::Link _links_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(links_lengthT > links_length)
        this->links = (rtabmap_msgs::Link*)realloc(this->links, links_lengthT * sizeof(rtabmap_msgs::Link));
      links_length = links_lengthT;
      for( uint32_t i = 0; i < links_length; i++){
      offset += this->st_links.deserialize(inbuffer + offset);
        memcpy( &(this->links[i]), &(this->st_links), sizeof(rtabmap_msgs::Link));
      }
     return offset;
    }

    virtual const char * getType() override { return "rtabmap_msgs/MapGraph"; };
